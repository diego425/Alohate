                        ...
                        <tr class="">
                            ...
                            <td data-label="Comprobante">
                                ...
                                <div class="gallery-container" id="gallery{{ $key }}">
                                    <img src="{{ asset('uploads/pagos/' . $pago->Foto_comprobante_pago) }}" alt="..." class="img-thumbnail">
                                </div>
                                @else
                                No encontrado
                                @endif
                            ...
                            <td class="flex" data-label="Confirmar">
                                <form action="{{ route('pagos.updatepago', [$pago->Id_pago_renta, $pago->Id_cobro_renta, 'Confirmar']) }}" method="post" class="formulario">
                                    @csrf
                                    <input type="hidden" name="tipoLocacion" value="{{$cobros[0]->tipoLocacion}}">
                                    @if (!empty($cobros[0]->lugarEspecifico[0]["id"]))
                                        <input type="hidden" name="id" value="{{$cobros[0]->lugarEspecifico[0]["id"]}}">
                                    @endif
                            ...
                            <td class="flex" data-label="Cancelar">
                                <form action="{{ route('pagos.updatepago', [$pago->Id_pago_renta, $pago->Id_cobro_renta, 'Cancelar']) }}" method="post" class="formulario">
                                    @csrf
                                    <input type="hidden" name="tipoLocacion" value="{{$cobros[0]->tipoLocacion}}">
                                    @if (!empty($cobros[0]->lugarEspecifico[0]["id"]))
                                        <input type="hidden" name="id" value="{{$cobros[0]->lugarEspecifico[0]["id"]}}">
